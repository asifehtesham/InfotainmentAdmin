...
export class SocialMediaTypeService {
  ...
  add(socialMediaType: SocialMediaType) {
    ...
    return this.http.post<any>(environment.infotApiUrl + action, socialMediaType)
      .pipe(map(data => {
        return data;
      }));
  }
  ...
  update(socialMediaType: SocialMediaType) {
    var action = 'socialMediaType';
    return this.http.put<any>(environment.infotApiUrl + action, socialMediaType)
      .pipe(map(data => {
  ...
  loadByID(id: number): Observable<SocialMediaType> {
    return this.http.get<any>(`${environment.infotApiUrl}socialMediaType/${id}`)
      .pipe(
        map(data => {
  ...
  loadData(index: number = 1, limit: number = 10): Observable<SocialMediaType[]> {
    ...
    return this.http.get<any>(`${environment.infotApiUrl}socialMediaType?index=${index}&limit=${limit}`)
      .pipe(
        map(data => {
  ...
  search(key: string): Observable<SocialMediaType[]> {
    return this.http.get<any>(`${environment.infotApiUrl}socialMediaType/search/${key}`)
      .pipe(
        map(data => {
          var socialMediaType = [];
          data.forEach(item => {
