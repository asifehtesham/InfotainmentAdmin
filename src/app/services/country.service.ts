...
export class CountryService {
  ...
  add(country: Country) {
    var action = 'country';
    return this.http.post<any>(environment.apiUrl + action, country)
      .pipe(map(data => {
        return data;
      }));
  }
  ...
  update(country: Country) {
    var action = 'country';
    return this.http.put<any>(environment.apiUrl + action, country)
      .pipe(map(data => {
  ...
  loadData(index: number = 1, limit: number = 10): Observable<Country[]> {
    ...
    return this.http.get<any>(`${environment.apiUrl}country?index=${index}&limit=${limit}`)
      .pipe(
        map(data => {
  ...
  search(key: string): Observable<Country[]> {
    return this.http.get<any>(`${environment.apiUrl}country/search/${key}`)
      .pipe(
        map(data => {
          var country = [];
          data.forEach(item => {
