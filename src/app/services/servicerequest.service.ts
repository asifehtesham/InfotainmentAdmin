...
export class ServicerequestService {
  ...
  loadByID(id: number): Observable<Servicerequest> {
    return this.http.get<any>(`${environment.infotApiUrl}servicerequest/${id}`)
      .pipe(
        map(data => {
  ...
  loadData(index: number = 1, limit: number = 10): Observable<Servicerequest[]> {
    ...
    return this.http.get<any>(`${environment.infotApiUrl}servicerequest?index=${index}&limit=${limit}`)
      .pipe(
        map(data => {
  ...
  search(key: string): Observable<Servicerequest[]> {
    return this.http.get<any>(`${environment.infotApiUrl}servicerequest/search/${key}`)
      .pipe(
        map(data => {
          var servicerequest = [];
          data.forEach(item => {
  ...
  deleteAll(ids: string) {
    ...
    var action = "servicerequest?ids=" + ids;
    return this.http.delete<any>(environment.infotApiUrl + action)
      .pipe(map(data => {
        return data;
      }));
