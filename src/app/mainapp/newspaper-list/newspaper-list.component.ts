...
export class NewspaperListComponent {
  ...
  ngOnInit() {
    ...
    this.search.valueChanges.subscribe(
      value => {
        if (value.length == 0) {
          this.newsPaperService.loadData().pipe(map((results => {
            //return results;
        ...
        this.newsPaperService.search(value).subscribe(results => {
  ...
  loadData() {
    this.newsPaperService.loadData(this.index, this.limit).subscribe(results => {
      this.loadEmptyMsg = true;
      console.log('come to the subscriber');
      this.dataSource.data = results;
    });
  ...
  chkActive_changed(ambulance: any, $event: MatSlideToggleChange) {
    ...
    this.newsPaperService.editactive(ambulance).subscribe();

  }

  ondelete(newsPaper: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.newsPaperService.delete(newsPaper.id).subscribe(params => {
    ...
    Swal.fire({
      ...
      .then((willDelete) => {
        if (willDelete.isConfirmed) {
          ...
          this.newsPaperService.deleteAll(ids).subscribe(result => {
            if (result) {
  ...
  checkboxLabel(row?: NewsPaper): string {
    if (!row) {
  ...
  onAdd() {
    ...
    const dialogRef = this.dialog.open(NewspaperDetailComponent, {
      width: '650px',
      data: { id: 0 }
    });
  ...
  onEdit(data: any) {
    const dialogRef = this.dialog.open(NewspaperDetailComponent, {
      width: '650px',
      data: { id: data.id, newsPaper: data }
    });
    dialogRef.afterClosed().subscribe(result => {
      this.loadData();
    });
