...
//import * as tuiImageEditor from '../../../../node_modules/tui-image-editor/src';
//import * as tuiImageEditor from 'node_modules/tui-image-editor/src';
...
export type PluginOptions = {
   ...
   * }
   */
  onApply?: ((imageEditor: ImageEditor, imageModel: grapejs.Component) => void) | null;
...
const pluginImageEditor: grapejs.Plugin<PluginOptions> = (editor, options = {}) => {
  ...
  editor.Commands.add(commandId, {
    //@ts-ignore
    // run(editor: grapejs.Editor, sender?: any, opts?: Record<string, any>): any {
    ...
    run(ed, s, options = {}) {
      ...
      //Asif
      //const target = (options.target || ed.getSelected()) as grapejs.Component;
      const target = (ed.getSelected()) as grapejs.Component;
    ...
    //@ts-ignore
    stop(ed) {
      (this.imageEditor as ImageEditor)?.destroy();
    ...
    applyChanges(target: grapejs.Component) {
    ...
    uploadImage(imageEditor: ImageEditor, target: grapejs.Component) {
    ...
    applyToTarget(result: string, target: grapejs.Component) {
...
export default pluginImageEditor;