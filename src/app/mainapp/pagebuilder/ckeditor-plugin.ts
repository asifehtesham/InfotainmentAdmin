...
export type PluginOptions = {
     ...
     */
    customRte?: Partial<grapesjs.RichTextEditor>;
...
const pluginCKE: grapesjs.Plugin<PluginOptions> = (editor, options = {}) => {
    const opts: Required<PluginOptions> = {
        options: {
            ...
            //     { name: 'link', groups: ['forms'] },
            //     { name: 'bulletedList', groups: ['forms'] },
            //     { name: 'numberedList', groups: ['forms'] },
            //     { name: 'blockQuote', groups: ['forms'] },
        ...
        if (isString(ckeditor)) {
            if (hasWindow) {
                ...
                scriptEl.onload = () => {
                    ...
                    ck.dtd.$editable.span = 1;
                    ck.dtd.$editable.a = 1;
                    ck.dtd.$editable.strong = 1;
                    ck.dtd.$editable.em = 1;
                    ck.dtd.$editable.ol = 1;
    ...
    editor.setCustomRte({
        ...
        enable(el, rte?: CKE.editor) {
            ...
            rteToolbar.childNodes.forEach(child => {
                var child1 = child as HTMLDivElement;
            ...
            rte.on('instanceReady', () => {
                console.log("instanceReady raise");
    ...
    /*ck.on('rteToolbarPosUpdate', (pos: any) => {
        const { elRect } = pos;
