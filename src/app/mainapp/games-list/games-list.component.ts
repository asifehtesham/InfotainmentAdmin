...
export class GamesListComponent {
  ...
  public dataSource = new MatTableDataSource<Games>();
  selection = new SelectionModel<Games>(true, []);

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sortable: MatSort;

  constructor(private http: HttpClient, private gamesService: GamesService, private dialog: MatDialog, private snakbar: MatSnackBar) { }
  ...
  ngOnInit() {
    ...
    this.search.valueChanges.subscribe(
      value => {
        if (value.length == 0) {
          this.gamesService.loadData().pipe(map((results => {
            //return results;
        ...
        this.gamesService.search(value).subscribe(results => {
  ...
  loadData() {
    this.gamesService.loadData(this.index, this.limit).subscribe(results => {
      this.loadEmptyMsg = true;
      console.log('come to the subscriber');
      this.dataSource.data = results;
    });
  ...
  chkActive_changed(ambulance: any, $event: MatSlideToggleChange) {
    ...
    this.gamesService.editactive(ambulance).subscribe();
  ...
  ondelete(games: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.gamesService.delete(games.id).subscribe(params => {
    ...
    Swal.fire({
      ...
      .then((willDelete) => {
        if (willDelete.isConfirmed) {
          ...
          this.gamesService.deleteAll(ids).subscribe(result => {
            if (result) {
  ...
  checkboxLabel(row?: Games): string {
    if (!row) {
  ...
  onAdd() {
    ...
    const dialogRef = this.dialog.open(GamesDetailComponent, {
      width: '650px',
      data: { id: 0 }
    });
