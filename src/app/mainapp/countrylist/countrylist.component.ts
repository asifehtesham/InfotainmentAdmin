...
export class CountrylistComponent {
  ...
  ngOnInit() {
    ...
    this.search.valueChanges.subscribe(
      value => {
        if (value.length == 0) {
          this.countryService.loadData().pipe(map((results => {
            //return results;
        ...
        this.countryService.search(value).subscribe(results => {
  ...
  chkActive_changed(ambulance: any, $event: MatSlideToggleChange) {
    ...
    this.countryService.editactive(ambulance).subscribe();
  ...
  ondelete(country: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.countryService.delete(country.id).subscribe(params => {
    ...
    Swal.fire({
      ...
      .then((willDelete) => {
        if (willDelete.isConfirmed) {
          ...
          this.countryService.deleteAll(ids).subscribe(result => {
            if (result) {
  ...
  onAdd() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      // this.snakbar.open('Country has been created successfully.', 'Dismise', {
      //   duration: 3000,
      //   horizontalPosition: 'right',
      //   verticalPosition: 'top',
      // });
  ...
  onEdit(data: any) {
    const dialogRef = this.dialog.open(CountrydetailComponent, {
      width: '650px',
      data: { id: data.id, country: data }
    });
    dialogRef.afterClosed().subscribe(result => {

      // this.snakbar.open('Country has been updated successfully.', 'Dismise', {
      //   duration: 3000,
      //   horizontalPosition: 'right',
      //   verticalPosition: 'top',
      // });
