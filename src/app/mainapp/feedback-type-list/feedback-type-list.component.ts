...
@Component({
  selector: 'app-feedback-type-list',
  templateUrl: './feedback-type-list.component.html',
  styleUrls: ['./feedback-type-list.component.scss']
})
export class FeedbackTypeListComponent {
  ...
  public dataSource = new MatTableDataSource<FeedbackType>();
  selection = new SelectionModel<FeedbackType>(true, []);

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sortable: MatSort;

  constructor(private http: HttpClient, private feedbackTypeService: FeedbackTypeService, private dialog: MatDialog, private snakbar: MatSnackBar) { }
  ...
  ngOnInit() {
    ...
    this.search.valueChanges.subscribe(
      value => {
        if (value.length == 0) {
          this.feedbackTypeService.loadData().pipe(map((results => {
            //return results;
        ...
        this.feedbackTypeService.search(value).subscribe(results => {
  ...
  loadData() {
    this.feedbackTypeService.loadData(this.index, this.limit).subscribe(results => {
      this.loadEmptyMsg = true;
      console.log('come to the subscriber');
      this.dataSource.data = results;
    });
  ...
  chkActive_changed(ambulance: any, $event: MatSlideToggleChange) {
    ...
    this.feedbackTypeService.editactive(ambulance).subscribe();

  }

  ondelete(feedbackType: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.feedbackTypeService.delete(feedbackType.id).subscribe(params => {
    ...
    Swal.fire({
      ...
      .then((willDelete) => {
        if (willDelete.isConfirmed) {
          ...
          this.feedbackTypeService.deleteAll(ids).subscribe(result => {
            if (result) {
  ...
  checkboxLabel(row?: FeedbackType): string {
    if (!row) {
  ...
  onAdd() {
    ...
    const dialogRef = this.dialog.open(FeedbackTypeDetailComponent, {
      width: '650px',
      data: { id: 0 }
    });
  ...
  onEdit(data: any) {
    const dialogRef = this.dialog.open(FeedbackTypeDetailComponent, {
      width: '650px',
      data: { id: data.id, feedbackType: data }
    });
    dialogRef.afterClosed().subscribe(result => {
      this.loadData();
    });
