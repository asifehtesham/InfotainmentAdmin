...
@Component({
  selector: 'app-social-media-type-list',
  templateUrl: './social-media-type-list.component.html',
  styleUrls: ['./social-media-type-list.component.scss']
})
export class SocialMediaTypeListComponent {
  ...
  public dataSource = new MatTableDataSource<SocialMediaType>();
  selection = new SelectionModel<SocialMediaType>(true, []);

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sortable: MatSort;

  constructor(private http: HttpClient, private socialMediaTypeService: SocialMediaTypeService, private dialog: MatDialog, private snakbar: MatSnackBar) { }
  ...
  ngOnInit() {
    ...
    this.search.valueChanges.subscribe(
      value => {
        if (value.length == 0) {
          this.socialMediaTypeService.loadData().pipe(map((results => {
            //return results;
        ...
        this.socialMediaTypeService.search(value).subscribe(results => {
  ...
  loadData() {
    this.socialMediaTypeService.loadData(this.index, this.limit).subscribe(results => {
      this.loadEmptyMsg = true;
      console.log('come to the subscriber');
      this.dataSource.data = results;
    });
  ...
  chkActive_changed(ambulance: any, $event: MatSlideToggleChange) {
    ...
    this.socialMediaTypeService.editactive(ambulance).subscribe();

  }

  ondelete(socialMediaType: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.socialMediaTypeService.delete(socialMediaType.id).subscribe(params => {
    ...
    Swal.fire({
      ...
      .then((willDelete) => {
        if (willDelete.isConfirmed) {
          ...
          this.socialMediaTypeService.deleteAll(ids).subscribe(result => {
            if (result) {
  ...
  checkboxLabel(row?: SocialMediaType): string {
    if (!row) {
  ...
  onAdd() {
    ...
    const dialogRef = this.dialog.open(SocialMediaTypeDetailComponent, {
      width: '650px',
      data: { id: 0 }
    });
  ...
  onEdit(data: any) {
    const dialogRef = this.dialog.open(SocialMediaTypeDetailComponent, {
      width: '650px',
      data: { id: data.id, socialMediaType: data }
    });
    dialogRef.afterClosed().subscribe(result => {
      this.loadData();
    });
