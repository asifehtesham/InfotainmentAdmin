...
export class NewslistComponent {
  ...
  constructor(private http: HttpClient, private newsService: NewsService, private dialog: MatDialog, private snakbar: MatSnackBar) { }
  ...
  ngOnInit() {
    ...
    this.search.valueChanges.subscribe(
      value => {
        if (value.length == 0) {
          this.newsService.loadData().pipe(map((results => {
            //return results;
        ...
        this.newsService.search(value).subscribe(results => {
  ...
  loadData() {
    this.newsService.loadData(this.index, this.limit).subscribe(results => {
      this.loadEmptyMsg = true;
      console.log('come to the subscriber');
      this.dataSource.data = results;
    });
  ...
  chkActive_changed(ambulance: any, $event: MatSlideToggleChange) {
    ...
    this.newsService.editactive(ambulance).subscribe();
  ...
  ondelete(news: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.newsService.delete(news.id).subscribe(params => {
    ...
    Swal.fire({
      ...
      .then((willDelete) => {
        if (willDelete.isConfirmed) {
          ...
          this.newsService.deleteAll(ids).subscribe(result => {
            if (result) {
  ...
  onAdd() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      // this.snakbar.open('News has been created successfully.', 'Dismise', {
      //   duration: 3000,
      //   horizontalPosition: 'right',
      //   verticalPosition: 'top',
      // });
  ...
  onEdit(data: any) {
    const dialogRef = this.dialog.open(NewsdetailComponent, {
      width: '650px',
      data: { id: data.id, news: data }
    });
    dialogRef.afterClosed().subscribe(result => {

      // this.snakbar.open('News has been updated successfully.', 'Dismise', {
      //   duration: 3000,
      //   horizontalPosition: 'right',
      //   verticalPosition: 'top',
      // });
